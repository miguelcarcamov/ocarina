...
class PolCalibration(object):
    ...
    def setUnknownModel(self, pol_source_object=None, field="", gaintable="", referencefield="", transferfield="", fitorder=1, usescratch=False):
        ...
        print(coeffs)
        print(flux_at_nu_0)
        print(np.exp(coeffs[0]))

        pol_source_object.setCoeffs(coeffs)
        spec_idx = coeffs.pop(0)
        print(spec_idx)
        ...
        source_dict = setjy(vis=self.vis, field=field, standard='manual', spw='', fluxdensity=[flux_at_nu_0,0,0,0], spix=spec_idx, reffreq=str(self.nu_0/1e9)+"GHz", interpolation="nearest", scalebychan=True, usescratch=usescratch)
        print(source_dict)
